...
A CLI tool that generates mock infrastructure for local Airflow development with the Astro CLI. Describe the external services your DAGs need in plain English, and the agent writes the correct `docker-compose.override.yml` and `airflow_settings.yaml` for you.
...
This tool handles all of that. You say what you need, it writes the config.
...
# Interactive mode: have a conversation, add services incrementally
...
4. Puts every service on the `airflow` network so your DAGs can reach them
5. Picks host ports that don't collide with Airflow's own services
...
- **Non-destructive** — Existing services and connections are never removed, only added or updated in place.
...
- **Dry run** — Use `--dry-run` to preview the full YAML output before committing to anything.
- **Sandboxed** — The agent can only read and write the two config files listed above. It cannot execute shell commands or access anything else on your system.
...
**Want to undo changes** — Check the `.bak` files created alongside your config. Rename `docker-compose.override.yml.bak` back to `docker-compose.override.yml` to restore the previous state.
